...
export const register = ({ email, password }) =>
  new Promise(async (resolve, reject) => {
    try {
      ...
      // Mã hoá id, email, role để lấy ra 1 đoạn token
      const accessToken = response[1]
        ...
        : null;
      // JWT_SECRET_REFRESH_TOKEN => không cần thông tin đăng nhập nên chỉ cần mã hoá id mà thôi
  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      const accessToken = isChecked
        ? jwt.sign({ id: response.id, email: response.email, role_code: response.role_code }, process.env.JWT_SECRET, {
            expiresIn: "5s",
          })
        : null;
